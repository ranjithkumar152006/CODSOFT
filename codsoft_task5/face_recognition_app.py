...
class FaceRecognitionApp:
    ...
    def load_known_faces(self):
        ...
        for filename in os.listdir(self.known_faces_dir):
            if filename.endswith((".jpg", ".png", ".jpeg")):
                ...
                # Optimization: Resize large reference images for faster encoding
                h, w = image.shape[:2]
                if w > 1000:
                    scale = 1000 / w
    ...
    def run_on_image(self, image_path):
        ...
        # Optimization: Use CNN on an optimized resolution (800px)
        # This provides the precision of Deep Learning but runs in seconds/minutes instead of hours.
        scale = 1.0
        target_width = 800
        if w > target_width:
            scale = target_width / w
            detection_image = cv2.resize(rgb_image, (0, 0), fx=scale, fy=scale)
        else:
            detection_image = rgb_image

        print(f"Scanning image (High Precision)...")
        
        try:
            # Restore CNN for exact face localization and high recognition rate.
            # Upsample=1 ensures small faces are detected properly.
            face_locations = face_recognition.face_locations(detection_image, number_of_times_to_upsample=1, model="cnn")
        except Exception:
            # Fallback only if CNN is unavailable
            face_locations = face_recognition.face_locations(detection_image, number_of_times_to_upsample=1, model="hog")
            
        # Scale back face locations with floating-point precision before rounding to avoid offset
        if scale != 1.0:
            face_locations = [(int(t/scale), int(r/scale), int(b/scale), int(l/scale)) for (t, r, b, l) in face_locations]

        # Final encoding with 1 jitter for stable recognition
        ...
        if num_faces > 1:
            ...
            face_locations = [(t, r + pad_w, b, l + pad_w) for (t, r, b, l) in face_locations]
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            # Dynamic margins: Tightened for "Exactly on the face" look
            height = bottom - top
            width = right - left
            
            # Use tighter margins (20% top instead of 60%) to center exactly on faces
            top = max(0, int(top - 0.20 * height))
            bottom = min(image.shape[0], int(bottom + 0.10 * height))
            left = max(0, int(left - 0.10 * width))
            right = min(image.shape[1], int(right + 0.10 * width))
            ...
            # Efficiently find best match using face distances
